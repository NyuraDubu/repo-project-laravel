    ...
    <div id="mobileMenu" class="fixed inset-0 z-[60] hidden md:hidden">
        ...
        <nav class="absolute right-0 top-0 h-full w-80 max-w-[85%] bg-white p-6 shadow-xl flex flex-col">
            <div class="flex items-center justify-between">
                ...
                <button id="menuClose" class="h-10 w-10 inline-flex items-center justify-center text-2xl" aria-label="{{ __('nav.close') }}">×</button>
            </div>

            <ul class="mt-6 space-y-4 text-sm">
                <li><a href="{{ url('/') }}" class="block py-2 font-semibold">{{ __('nav.home') }}</a></li>
                <li><a href="{{ route('work') }}" class="block py-2 font-semibold">{{ __('nav.work') }}</a></li>
            </ul>

            <nav class="hidden md:flex items-center gap-4">
                <a href="{{ route('lang.switch', ['locale' => 'fr']) }}" class="{{ app()->getLocale()==='fr' ? 'font-bold text-black' : 'text-[#B2B2B2]' }}">{{ __('nav.fr') }}</a>
                <a href="{{ route('lang.switch', ['locale' => 'en']) }}" class="{{ app()->getLocale()==='en' ? 'font-bold text-black' : 'text-[#B2B2B2]' }}">{{ __('nav.en') }}</a>
            </nav>
        </nav>
    </div>
@endsection
...
@section('footer')
    <footer class="section-white">
        <div class="container-page py-8 sm:py-10 grid gap-6 grid-cols-1 sm:grid-cols-[auto_1fr_auto] items-center">
            <a href="#" target="_blank" rel="noopener" class="order-1 sm:order-2 btn btn-outline font-medium w-full sm:w-auto border border-black rounded-sm text-center mx-auto sm:mx-0 sm:justify-self-center text-black">
            ...
            <a href="#top" aria-label="{{ __('common.back_to_top') }}" class="order-3 sm:order-3 group inline-flex h-16 w-16 items-center justify-center rounded-full border-2 border-black text-black transition hover:bg-black hover:text-white fixed bottom-6 right-6 z-50 sm:static sm:justify-self-end" onclick="event.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' });">
                <svg viewBox="0 0 24 24" class="h-6 w-6 transition-transform group-hover:-translate-y-0.5" fill="none" stroke="currentColor" stroke-width="2">
